...
<header>
    ...
    if (Yii::$app->user->isGuest) {
        ...
        echo Html::tag('li', Html::a('My Saved Books', ['/savedbooks/show-my-saved-books', 'id' => $user->identity->id], ['class' => 'dropdown-item']));
        echo Html::tag('li', Html::a('My Expected Returns', ['/takenbooks/show-my-taken-books', 'id' => $user->identity->id], ['class' => 'dropdown-item']));
        echo Html::tag('li', Html::a('My Return History', ['/returnedbooks/show-my-returned-books', 'id' => $user->identity->id], [
            'class' => 'dropdown-item',
            'data' => ['method' => 'post'],
        ]));
                            ...
                            <div class="card-body pb-1 pt-1">
                                <div class="col-12 text-center mt-1">
                                    <?php if ($selected_user->is_active == 1): ?>
                                        <?= Html::a('<span class="bi bi-pencil-fill"></span>', ['update', 'id' => $selected_user->id], ['class' => 'btn btn-warning btn-outline-dark btn-responsive btn-sm mt-1']) ?>
                                        <?php if ($user_id_in_basket == null): ?>
...
<script>
    $(document).ready(function () {
        // Show the flash message initially (if available)
        $(".alert").fadeIn();

        // Add a timeout to close the flash message with fade effect after one second
        setTimeout(function () {
            $(".alert").fadeOut("slow", function () {
                // Optional: Remove the flash message from the DOM after fadeOut animation completes
                $(this).remove();
            });
